export default class Word {
	constructor({ label, speed }) {
		...
		linkEl.setAttribute('href', 'projects.html')
			...
			if (this.position.x <= 0)
				this.setVelocityAngle(
					Math.random() > 0.5
						...
						: Math.PI * 1.5 + Math.random() * Math.PI * 0.5,
				)
			else if (this.position.x >= 1) this.setVelocityAngle(Math.PI * 0.5 + Math.random() * Math.PI)
			else if (this.position.y <= 0) this.setVelocityAngle(Math.random() * Math.PI)
			else if (this.position.y >= 1) this.setVelocityAngle(Math.PI + Math.random() * Math.PI)
		}
	}
	...
	set opacity(value) {
		this._opacity = value
		if (this.el) this.el.style.opacity = value
	}
